...
* [util](#util)
    ...
    * [.funct](#util.funct)
        * [.retry(fct, args, [retries], [interval], [scope])](#util.funct.retry)
        * [.executeOptional(fct, args)](#util.funct.executeOptional)
...
<a name="util.funct.retry"></a>

#### funct.retry(fct, args, [retries], [interval], [scope])
Retries the passed function n times with a specific interval until it executed successfully.

**Kind**: static method of [<code>funct</code>](#util.funct)  
...
**Example**  
```js
await util.funct.retry(ui5.userInteraction.fill, [selector, value], 4, 10000);
```
**Example**  
```js
await util.funct.retry(async () => {
 await ui5.userInteraction.fill(selector, "ABC");
}, [], 2, 30000);
```
<a name="util.funct.executeOptional"></a>

#### funct.executeOptional(fct, args)
...
**Example**  
```js
await util.funct.executeOptional(ui5.userInteraction.fill, [selector, value]);
```
**Example**  
```js
await util.funct.executeOptional(async () => {
 await ui5.userInteraction.fill(selector, "ABC");
}, []);
```
